...
unsigned char compressed_pubkey1[65];
unsigned char coresul[50000*65];
/* 65*x */
memset(coresul, 0, 65*50000);
secp256k1_ec_pubkey_create(ctx, &pubkey1, key);
secp256k1_ec_pubkey_create(ctx, &pubkey2, key);
len = sizeof(compressed_pubkey1);
secp256k1_ec_pubkey_serialize(ctx, compressed_pubkey1, &len, &pubkey1, SECP256K1_EC_UNCOMPRESSED);
    printf("1Compressed Pubkey1: ");
    print_hex(compressed_pubkey1, sizeof(compressed_pubkey1));
    size_t i,b;
    seconds = time(NULL);
/*     for (i = 0; i < 5000000; i++) {
        secp256k1_ec_pubkey_pub_add(ctx, &pubkey1, &pubkey2);
    } */
    b = 0;
    for (i = 0; i < 400000; i++) {
        secp256k1_ec_pubkey_pub_add_batch(ctx, coresul, &pubkey1, &pubkey2);
        b += 50000;
        if(b > 50000000){break;}
    }
secp256k1_ec_pubkey_serialize(ctx, compressed_pubkey1, &len, &pubkey1, SECP256K1_EC_UNCOMPRESSED);
    printf("2Compressed Pubkey1: ");
    print_hex(compressed_pubkey1, sizeof(compressed_pubkey1));
    
    seconds = time(NULL)-seconds;
    printf("\nactual   count = %zu\n", b);
    printf("pubkey total = %12.8f\n", (double)seconds);
    printf("pubkey avg   = %12.8f\n", (double)seconds / (double)b);
    printf("pubkey/sec   = %12.2f\n\n", (double)b / (double)seconds);
    /* print_hex(coresul, sizeof(coresul)); */
    return 0;
}
